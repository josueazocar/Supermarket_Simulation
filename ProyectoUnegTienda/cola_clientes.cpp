...
void seleccionarDatosAleatorio(std::set<int>& indices_usados) {
	...
	if (indices_usados.size() >= 49) { //Si los elementos del conjunto llegan a 49 entonces este se limpiara
		indices_usados.clear();  // Reiniciamos el conjunto
	}

	do {
		indice1 = rangoAleatorio(rng); // Generar un índice aleatorio
	} while (indices_usados.find(indice1) != indices_usados.end()); // Verificar si el índice ya fue usado
	indices_usados.insert(indice1); // Agregar el índice al conjunto
	indice_random = indice1; // Asignar el índice aleatorio a la variable global
}
...
int TamanoCola() {
	...
	tamano = cola_clientes.size(); //Obtener el tamaño de la cola
	return tamano;
}

bool verificarCarritoLleno(int indice_carrito) {
	return  carritos_llenos[indice_carrito]; //Verificar si el carrito esta lleno
}

void AgregarClienteCola(int indice_carrito) { //Agregar cliente a la cola
	if (verificarCarritoLleno(indice_carrito)) { //Verificar si el carrito esta lleno
...
void QuitarClienteCola(int indice_carrito) {
	...
	if (!verificarCarritoLleno(indice_carrito)) { //Verificar si el carrito esta lleno
		if (!cola_clientes.empty()) { //Verificar si la cola no esta vacia
			cola_clientes.pop();
...
int tiempo_aletorio_clientes(int tiempo_generacionMinimo, int tiempo_generacionMaximo) {
	std::uniform_int_distribution<int> tiempo_clientes(tiempo_generacionMinimo, tiempo_generacionMaximo); //Generar un tiempo aleatorio
...
bool clientes_haciendo_compras() { //Verificar si hay clientes haciendo compras
	for (int i = 0; i < 6; i++) {
